# ENTSO-E API works in Brussels timezone. Ljubljana timezone has the same offset.
...
# copied from entsoe-py github repository
...
CWE_R =         '10YDOM-REGION-1V', 'CWE Region, ',                                   'Europe/Brussels',
...
GB =            '10YGB----------A', 'Great Britain, BZ / CA/ MBA',                   'Europe/London',
...
# example: ("DE_50HZ", "10YDE-VE-------2", "50Hertz …", "Europe/Berlin")
BIDDING_ZONE_ENTRIES1 = [
    (name,) + value
    for name, value in globals().items()
    if name.isupper() and isinstance(value, tuple) and len(value) >= 3
]

BIDDING_ZONES1 = {long_name : (eic_code, country_code) for country_code, eic_code, long_name, _ in BIDDING_ZONE_ENTRIES1 if any(word == l for l in long_name.split(sep=",")[1].split(sep=" ") for word in (("BZ","BZN")))}
BIDDING_ZONES = {long_name.split(sep=",")[0].replace(",", "").strip(): pair for long_name, pair in BIDDING_ZONES1.items()} 

COUNTRIES1 = {long_name : (eic_code, country_code) for country_code, eic_code, long_name, _ in BIDDING_ZONE_ENTRIES1 if len(country_code) <= 3}
COUNTRIES = {long_name.split(sep=",")[0].replace(",", "").strip(): pair for long_name, pair in COUNTRIES1.items()} 

CONTROL_AREAS = {long_name : (eic_code, country_code) for country_code, eic_code, long_name, _ in BIDDING_ZONE_ENTRIES1 if any(word == l for l in long_name.split(sep=",")[1].split(sep=" ") for word in (("CA","CA,","CA/")))}
CONTROL_AREAS= {long_name.split(sep=",")[0].replace(",", "").strip(): pair for long_name, pair in CONTROL_AREAS.items()}